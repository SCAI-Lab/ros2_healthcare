...
class SensomativeDriver(Node):
    def __init__(self, address, hci_mac=None):
        super().__init__('sensomative_driver')
        ...
        if hci_mac is not None:
            for adapt in all_adapters:
                if adapt.address() == hci_mac.upper():
                    adapter = adapt
                    break
        if adapter is None:
